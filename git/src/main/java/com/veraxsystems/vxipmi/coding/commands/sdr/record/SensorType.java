...
public enum SensorType {
	...
	/**
	 * CoolingDevice冷却设备
	 * OtherUnitsBasedSensor 其他基于单元的传感器
	 * Memory内存
	 * DriveBay驱动器槽
	 * PostMemoryResize后内存大小
	 * 系统固件进步(SensorType.SYSTEMFIRMWAREPROGESS),
事件日志记录残疾(SensorType.EVENTLOGGINGDISABLED),
引导错误(SensorType.BOOTERROR),
看狗(SensorType.WATCHDOG1),
操作系统引导(SensorType.OSBOOT),
系统事件(SensorType.SYSTEMEVENT),
操作系统停止(SensorType.OSSTOP),
关键的中断(SensorType.CRITICALINTERRUPT),
槽连接器(SensorType.SLOTCONNECTOR),
系统Acpi电源状态(SensorType.SYSTEMACPIPOWERSTATE)，
看dog2(SensorType.WATCHDOG2),
平台警报(SensorType.PLATFORMALERT),
实体的存在(SensorType.ENTITYPRESENCE),
监控AsicIc(SensorType.MONITORASICIC),
局域网(SensorType.LAN),
温度(SensorType.TEMPERATURE),
电压(SensorType.VOLTAGE),
当前(SensorType.CURRENT),
风扇(SensorType.FAN),
物理安全(SensorType.PHYSICALSECURITY),
平台安全违规尝试(sensortype . Platform securityviolationtry)，
处理器(SensorType.PROCESSOR),
电源(SensorType.POWERSUPPLY),
动力单元(SensorType.POWERUNIT),
按钮开关(SensorType.BUTTONSWITCH),
模块板(SensorType.MODULEBOARD),
单片机协处理器(SensorType.MICROCONTROLLERCOPROCESSOR),
外接程序卡(SensorType.ADDINCARD),
底盘(SensorType.CHASSIS),
芯片组(SensorType.CHIPSET),
OtherFru (SensorType.OTHERFRU),
健康管理子系统(SensorType.MANAGEMENTSUBSYSTEMHEALTH),
电缆互连(SensorType.CABLEINTERCONNECT),
电池(SensorType.BATTERY),
《终结者》(SensorType.TERMINATOR),
会话审计(SensorType.SESSIONAUDIT),
系统启动(SensorType.SYSTEMBOOT),
版本变化(SensorType.VERSIONCHANGE),
Fru状态(SensorType.FRUSTATE),
Oem(SensorType.OEM),
Oem保留(SensorType.OEMRESERVED)
