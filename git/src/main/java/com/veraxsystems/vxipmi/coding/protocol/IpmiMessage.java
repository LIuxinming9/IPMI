...
public abstract class IpmiMessage {
	...
	public String toString() {
		return "IpmiMessage [authenticationType=" + authenticationType + ", sessionSequenceNumber="
				+ sessionSequenceNumber + ", sessionID=" + sessionID + ", authCode=" + Arrays.toString(authCode)
				+ ", payload=" + payload + ", payloadLength=" + payloadLength + ", confidentialityAlgorithm="
				+ confidentialityAlgorithm + "]";
	}

	private AuthenticationType authenticationType;

    private int sessionSequenceNumber;//会话序列号
    
    private int sessionID;

    private byte[] authCode;//授权码

    private IpmiPayload payload;//有效载荷
    
    private int payloadLength;//有效载荷长度
