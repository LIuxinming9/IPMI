 ...
 * 此记录用于在IPMB或管理控制器私有总线上存储设备的位置和类型信息，
 * 这些总线既不是IPMI FRU设备，也不是IPMI管理控制器。
 */
public class GenericDeviceLocatorRecord extends SensorRecord {

	@Override
	public String toString() {
		return "GenericDeviceLocatorRecord [deviceAccessAddress=" + deviceAccessAddress + ", deviceSlaveAddress="
				+ deviceSlaveAddress + ", channelNumber=" + channelNumber + ", accessLun=" + accessLun + ", busId="
				+ busId + ", addressSpan=" + addressSpan + ", deviceType=" + deviceType + ", deviceTypeModifier="
				+ deviceTypeModifier + ", entityId=" + entityId + ", entityInstance=" + entityInstance + ", name="
				+ name + "]";
	}
	...
	private int channelNumber;//频道数

	private int accessLun;//访问硬盘

	private int busId;//母线标识 
