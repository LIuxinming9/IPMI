...
/**
 * FRU record containing Board info包含板信息的财务报表记录. <br>
 ...
 * 此区域提供关于FRU信息设备所在的板的序列号、部件号和其他信息。
 */
public class BoardInfo extends FruRecord {

	@Override
	public String toString() {
		return "BoardInfo [mfgDate=" + mfgDate + ", boardManufacturer=" + boardManufacturer + ", boardProductName="
				+ boardProductName + ", boardSerialNumber=" + boardSerialNumber + ", boardPartNumber=" + boardPartNumber
				+ ", fruFileId=" + Arrays.toString(fruFileId) + ", customBoardInfo=" + Arrays.toString(customBoardInfo)
				+ "]";
	}

	private Date mfgDate;

	private String boardManufacturer = "";//板制造商

	private String boardProductName = "";

	private String boardSerialNumber = "";//板序号

	private String boardPartNumber = "";//板零件号

	private byte[] fruFileId = new byte[0];

	private String[] customBoardInfo = new String[0];//定制板信息
	
	private static Logger logger = Logger.getLogger(BoardInfo.class);

	/**
	 * Creates and populates填充 record
	 * 
	 * @param fruData
