...
public enum CompletionCode {
	...
	* FRU设备忙。由于逻辑FRU设备的实现处于FRU信息暂时不可用的状态，因此无法完成请求。
	* 如果FRU是作为共享总线上的设备实现的，那么这可能是由于诸如仲裁los s之类的条件。
	*/
	Frudevicebusy(CompletionCode.FRUDEVICEBUSY),
	/**
	* Unauthorized name.未经授权的名字。
	*/
	...
	* 的空间。无法完成命令，原因是执行给定命令操作所需的存储空间不足。
	*/
	...
	* 无法执行重复的请求。此完成代码用于无法返回为请求的原始实例返回的响应的设备。
	* 这些设备应该提供单独的命令，以确定原始请求的完成状态。事件接收器不使用此完成代码，
	* 但在对(有效的)重复请求的响应中返回00h完成代码。
	*/
	DuplicatedRequest(CompletionCode.DUPLICATEDREQUEST),
	/**
	* Command response could not be provided. SDR Repository in update mode.
	* 无法提供命令响应。SDR存储库处于更新模式。
	*/
	...
	* 目的地不可用。无法将请求发送到选定的目的地。例如，如果请求消息以SMS为目标，
	* 则可以返回此代码，但特定通道禁用接收消息队列接收。
	*/
	...
	* 由于权限级别不足或其他基于安全性的限制(例如“固件防火墙”禁用)，无法执行命令。
	*/
