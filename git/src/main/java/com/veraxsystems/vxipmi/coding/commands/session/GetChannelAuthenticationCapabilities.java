...
public class GetChannelAuthenticationCapabilities extends IpmiCommandCoder {
	...
	public String toString() {
		return "GetChannelAuthenticationCapabilities [requestedPrivilegeLevel=" + requestedPrivilegeLevel
				+ ", channelNumber=" + channelNumber + ", requestVersion=" + requestVersion + "]";
	}

	private PrivilegeLevel requestedPrivilegeLevel;//请求特权级别
	 ...
	 * 在1.5版本中启动解码类。将请求的权限级别设置为用户。
	 * 将通道号设置为14，表示响应将包含发出此请求的通道的信息。将会话参数设置为默认值。
	 * 
	 ...
	 * 初始化类。设置IPMI版本到版本。
	 * 将请求的权限级别设置为用户。将通道号设置为14，表示响应将包含发出此请求的通道的信息。
	 * 
	 * @param version
	 ...
	 *            如果用requestVersion = {@link IpmiVersion#V15}将Get通道
	 *            身份验证功能命令发送到BMC，它将作出响应，即使它支持IPMI v2.0，它也不支持。
	 ...
	 * 初始化类。设置IPMI版本到版本。设置要求的特权等级。将通道号设置为channelNumber。
	 * 
	 * @param version
	 *            - Version of IPMI protocol used
	 * @param requestVersion
