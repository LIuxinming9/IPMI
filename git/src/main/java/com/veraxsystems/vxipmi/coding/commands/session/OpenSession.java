...
public class OpenSession extends IpmiCommandCoder {
	...
	public String toString() {
		return "OpenSession [requestedPrivilegeLevel=" + requestedPrivilegeLevel + ", sessionID=" + sessionID + "]";
	}
	 ...
	 * {@link IpmiVersion#V20} since OpenSession is a RMCP+ command. Sets
	 * authentication type to RMCP+.
	 * 初始化用于编码和解码的类。设置IPMI版本为{@link IpmiVersion#V20}，
	 * 因为OpenSession是RMCP+命令。将身份验证类型设置为RMCP+。
	 * 
	 * @see IpmiVersion
	 * 
	 * @param sessionID
	 ...
	 *            point of session creation SIK is not yet known) be not
	 *            initialized.
	 *            密码套件可能(也可能是，因为在会话创建的这一点上SIK还不知道)没有初始化。
	 * @see CipherSuite
