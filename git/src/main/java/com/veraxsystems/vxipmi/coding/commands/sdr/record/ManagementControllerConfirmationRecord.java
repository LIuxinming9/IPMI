 ...
 * 这个记录可以被实用软件用来记录给定的控制器在系统中被发现。
 * 稍后，软件可以使用记录信息来确认仍然存在相同的控制器。
 */
public class ManagementControllerConfirmationRecord extends SensorRecord {

	@Override
	public String toString() {
		return "ManagementControllerConfirmationRecord [deviceSlaveAddress=" + deviceSlaveAddress + ", deviceId="
				+ deviceId + ", channelNumber=" + channelNumber + ", deviceRevision=" + deviceRevision
				+ ", firmwareRevision1=" + firmwareRevision1 + ", firmwareRevision2=" + firmwareRevision2
				+ ", ipmiVersion=" + ipmiVersion + ", manufacturerId=" + manufacturerId + ", productId=" + productId
				+ ", deviceGuid=" + Arrays.toString(deviceGuid) + "]";
	}

	private int deviceSlaveAddress;//设备从属地址

	private int deviceId;//设备号

	private int channelNumber;

	private int deviceRevision;//设备修改

	private int firmwareRevision1;//固件版本1
