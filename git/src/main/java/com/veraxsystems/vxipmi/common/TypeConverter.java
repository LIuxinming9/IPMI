...
public final class TypeConverter {
	 ...
	 * 在BigEndian约定中将int转换为字节数组。以Java字节表示形式编码无符号字节。
	 ...
	 * unsigned byte.
	 * 因为Java不支持无符号字节值，所以简单的类型转换会导致溢出和错误的值。
	 * 这个函数将int类型转换为伪无符号字节。
	 * @param value
	 ...
	 * 将BCD编码的字节转换为位7:4包含修订的最低有效位，位3:0包含最有效位。
	 * @param value
	 *            decoded value
	 ...
	 * GMT.
	 * 解码编码为从1970年1月1日00:00:00开始的秒数的日期。
	 */
