	...
	<script type="text/javascript">
		function queryData() 
		{
			...
			if(query){
				$.ajax({
			        type:"POST",
			        url:"${pageContext.request.contextPath}/warnhistory/query", 
		...
		$.post("${pageContext.request.contextPath}/warnhistory/changeInfo",{infoType:infoType},function(data){
			var IPType = "";
		...
		$("#nameType").empty();
			...
			$.post("${pageContext.request.contextPath}/warnhistory/changename",{queryType:queryType,IPType:IPType},function(data){
			var nameType = "";
		...
		$("#queryType").empty();
			...
			$.post("${pageContext.request.contextPath}/warnhistory/changetype",{IPType:IPType},function(data){
			var queryType = "";
		...
		function exportData() 
		{
			...
			var query = true;
			if(query){
				window.open("${pageContext.request.contextPath}/warnhistory/exprotList?&datemin=" + datemin + 
						"&datemax=" + datemax+ "&queryType=" + queryType + "&nameType=" + nameType + "&IPType=" + IPType);	
			}
		}
	</script>
	<section id='content'>
							...
							<table class="table table-border table-bordered table-hover table-bg table-sort" style="table-layout:fixed;">
								...
								<%-- <c:forEach items="${list}" var="item" varStatus="status">
					         		<tr class="text-c">
					                    ...
					                    <td class="center"><fmt:formatDate value="${item.end_time}" pattern="yyyy年MM月dd日  HH:mm:ss" /></td>
