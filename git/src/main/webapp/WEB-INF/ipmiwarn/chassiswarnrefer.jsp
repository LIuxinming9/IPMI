	...
	<script type="text/javascript">
		function queryData() 
		{
			...
			if(query){
				$.ajax({
			        type:"POST",
			        url:"${pageContext.request.contextPath}/chassiswarnrefer/query", 
		...
		function exportData() 
		{
			...
			var query = true;
			if(query){
				window.open("${pageContext.request.contextPath}/chassiswarnrefer/exprotList?&queryType=" + queryType + "&nameType=" + nameType + "&IPType=" + IPType);	
			}
		}
	</script>
	<section id='content'>
	        ...
	        <div class='row-fluid' id='content-wrapper'>
	            <div class='span12'>
	            	 <nav class="breadcrumb">
						<i class="Hui-iconfont">&#xe67f;</i>  告警管理 <span class="c-gray en">&gt;</span> 监控列表 <span class="c-gray en">&gt;</span> 机箱监控列表
					</nav> 
								...
								<c:forEach items="${list}" var="item" varStatus="status">
					         		<tr class="text-c">
					                    ...
					                    <c:choose>
										    ...
										    <c:otherwise>
										    	<td class="center">
					                             	<a href="${pageContext.request.contextPath}/chassiswarnrefer/add?IP=${item.IP}&name=${item.name}" class="btn btn-mini radius">
