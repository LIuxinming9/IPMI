	...
	<script type="text/javascript">
		function queryData() 
		{
			...
			if(query){
				$.ajax({
			        type:"POST",
			        url:"${pageContext.request.contextPath}/sensor/sensorvoquery", 
		...
		$("#nameType").empty();
			...
			$.post("${pageContext.request.contextPath}/sensor/changevoname",{IPType:IPType},function(data){
			var nameType = "";
				nameType += "<option values='" + "全部" + "'>" + "全部" + "</option>";
			for(var i=0;i<data.length;i++){
				nameType += "<option values='" + data[i].name + "' title='" + data[i].mapname + "'>" + data[i].name + "</option>";
			}
		...
		function currentData()
		{
			...
			if(query){
				$.ajax({
			        type:"POST",
			        url:"${pageContext.request.contextPath}/sensor/sensorvocurrentquery", 
		...
		function exportData() 
		{
			...
			if(query){
				window.open("${pageContext.request.contextPath}/sensor/exprotvoList?&datemin=" + datemin + "&datemax=" + datemax+ "&IPType=" + IPType+ "&nameType=" + nameType);
			}
		}
		...
		function exportcurrentData() 
		{
			...
			var query = true;
			if(query){
				window.open("${pageContext.request.contextPath}/sensor/exprotvocurrentList?&IPType=" + IPType+ "&nameType=" + nameType);
			}
		}
	</script>
	<section id='content'>
	        ...
	        <div class='row-fluid' id='content-wrapper'>
	            <div class='span12'>
	            	<nav class="breadcrumb">
						<i class="Hui-iconfont">&#xe67f;</i>  信息统计  <span class="c-gray en">&gt;</span> 传感器记录 <span class="c-gray en">&gt;</span> 电压
					</nav>
	            	<div class="page-container">
	            		<div class="text-c"> 
							...
							<div class="text-c"> 
								...
								<span class="select-box inline" style="padding-left:170px;"></span>
