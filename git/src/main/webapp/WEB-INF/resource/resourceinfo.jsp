	...
	<script type="text/javascript">
		...
		function queryData() 
		{
			...
			if(query){
				$.ajax({
			        type:"POST",
			        url:"${pageContext.request.contextPath}/resource/resourceDiv", 
		...
		$("#nameType").empty();
			...
			$.post("${pageContext.request.contextPath}/resource/changename",{IPType:IPType},function(data){
			var nameType = "";
		...
		function exportData() 
		{
			...
			var query = true;
			if(query){
				window.open("${pageContext.request.contextPath}/resource/exprotresourceList?&datemin=" + datemin + "&datemax=" + datemax+ "&queryType=" + queryType+ "&IPType=" + IPType+ "&nameType=" + nameType);	
			}
		}
	        ...
	        <div class='row-fluid' id='content-wrapper'>
	            <div class='span12'>
	            	<nav class="breadcrumb">
						<i class="Hui-iconfont">&#xe67f;</i>  信息统计 <span class="c-gray en">&gt;</span> 资源管理 
					</nav>
							...
							<table class="table table-border table-bordered table-hover table-bg table-sort" style="table-layout:fixed;">
								...
								<c:forEach items="${resource_name}" var="item" varStatus="status">
					         		<tr class="text-c">
					                    ...
					                    <td class="center">${item.hardnum}</td>
					                    <td class="center">${item.emptynum}</td>
					                </tr>
					            </c:forEach>
						       </tbody>
						   </table> 
