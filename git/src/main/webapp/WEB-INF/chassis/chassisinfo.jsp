	...
	<script type="text/javascript">
		function queryData() 
		{
			...
			if(query){
				$.ajax({
			        type:"POST",
			        url:"${pageContext.request.contextPath}/chassis/chassisquery", 
		...
		function querycurrentData()
		{
			...
			if(query){
				$.ajax({
			        type:"POST",
			        url:"${pageContext.request.contextPath}/chassis/chassiscurrentquery", 
		...
		function exportData() 
		{
			...
			if(query){
				window.open("${pageContext.request.contextPath}/chassis/exprotChassisList?&datemin=" + datemin + "&datemax=" + datemax+ "&IPType=" + IPType+ "&nameType=" + nameType);	
			}
		}
		...
		function exportcurrentData()
		{
			...
			var query = true;
			if(query){
				window.open("${pageContext.request.contextPath}/chassis/exprotChassiscurrentList?&IPType=" + IPType+ "&nameType=" + nameType);
			}
		}
	</script>
	<section id='content'>
	        ...
	        <div class='row-fluid' id='content-wrapper'>
	            <div class='span12'>
	            	<nav class="breadcrumb">
						<i class="Hui-iconfont">&#xe67f;</i>  信息统计 <span class="c-gray en">&gt;</span> 机箱状态
					</nav>
	            		...
	            		<div class="text-c">
							...
							<div class="text-c"> 
								...
								<select id="nameType" name="nameType" class="select" style="width:200px;">
									...
									<option value="is_power_fault">电源有故障</option>
									<option value="cooling_fault_detected">风扇有故障</option>
									<option value="drive_fault_detected">驱动器有故障</option>
									<option value="is_chassis_intrusion_active">机箱被入侵</option>
									<option value="is_interlock">检测到联锁</option>
									<option value="is_power_overload">系统因功率过载而关闭</option>
									<option value="is_power_on">电源已开启</option>
									<option value="ac_failed">AC失败</option>
									<option value="was_power_fault">上次断电是因为电源故障</option>
									<option value="was_interlock">上次断电是因为电源联锁</option>
								</select>
								</span>
								<span style="padding-left:20px;"></span>
								<button id="queryBtn" type="button" class="btn btn-success radius" onclick="queryData();"><i class="Hui-iconfont">&#xe665;</i> 历史数据查询</button>
								<span style="padding-left:20px;"></span>
								<button id="queryBtn" type="button" class="btn btn-success radius" onclick="querycurrentData();"><i class="Hui-iconfont">&#xe665;</i> 实时数据查询</button>
								<span style="padding-left:20px;"></span>
							...
							<table class="table table-border table-bordered table-hover table-bg table-sort" style="table-layout:fixed;">
								<thead>
									<tr class="text-c">
					                   ...
					                   <th width="100px">机箱名字</th>
					                   <th width="100px">机箱状态</th>
					                </tr>
					            </thead>
					            <tbody>
