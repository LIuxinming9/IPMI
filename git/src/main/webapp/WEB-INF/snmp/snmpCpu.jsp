	...
	<script type="text/javascript">
		function queryData() 
		{
			...
			if(query){
				$.ajax({
			        type:"POST",
			        url:"${pageContext.request.contextPath}/snmpCpu/query", 
		...
		$.post("${pageContext.request.contextPath}/sensorwarnset/changeInfo",{infoType:infoType},function(data){
			...
			for(var i=0;i<data.length;i++){
				IPType += "<option value='" + data[i].valueId + "' onclick='changename();'>" + data[i].value + "</option>";
			}
		...
		$("#nameType").empty();
			...
			$.post("${pageContext.request.contextPath}/snmpCpu/changename",{IPType:IPType},function(data){
			var nameType = "";
				nameType += "<option value='" + "全部" + "'>" + "全部" + "</option>";
			for(var i=0;i<data.length;i++){
				nameType += "<option value='" + data[i].name + "'>" + data[i].name + "</option>";
			}
		...
		function exportData()
		{
			...
			var query = true;
			if(query){
				window.open("${pageContext.request.contextPath}/snmpCpu/exprotList?&nameType=" + nameType + "&IPType=" + IPType);	
			}
		}
	</script>
	<section id='content'>
	        ...
	        <div class='row-fluid' id='content-wrapper'>
	            <div class='span12'>
	            	 <nav class="breadcrumb">
						<i class="Hui-iconfont">&#xe67f;</i>  信息统计 <span class="c-gray en">&gt;</span> 传感器记录  <span class="c-gray en">&gt;</span> CPU
					</nav> 
	            	<div class="page-container">
						<div class="text-c"> 
								...
								<span class="select-box inline" style="padding-left:20px;">
								OID描述：
								<select id="nameType" name="nameType" class="form-control" style="width:150px;">
								</select>
								</span>
							...
							<table class="table table-border table-bordered table-hover table-bg table-sort" style="table-layout:fixed;">
								<thead>
									<tr class="text-c">
					                   ...
					                   <th width="200px">OID</th>
					                   <th width="200px">OID描述</th>
					                   <th width="300px">读值</th>
					                </tr>
					            </thead>
					            <tbody>
						       </tbody>
